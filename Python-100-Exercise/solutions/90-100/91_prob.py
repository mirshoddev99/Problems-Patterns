"""Write a Python program to find all n-digit integers that start or end with 2. Go
to the editor
Input: 1
Output:
[2]
...
def test(n):
    if n <= 0:
        ...
        nums = list(range(2, 10 ** n))
        result = list(filter(lambda x: x % 10 == 2 or x // 10 == 2, nums))
        return result
